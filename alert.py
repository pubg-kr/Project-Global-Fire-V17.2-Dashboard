...
def check_market_status():
    ...
    try:
        ...
        _qqq_dev_series = (qqq_mo_data['Close'] / qqq_mo_data['MA120'] - 1.0).dropna()
        ...
        # [V24.6, 원칙 4-조건A] 이격도가 100% 미만으로 마감된 '연속 개월수' (최근값부터 역순 카운트)
        ...
        # [V24.6] 레벨 구분 삭제: 이격도 100% 방어 룰은 자산 규모(Level) 불문 전 Level 공통 발동.
        # is_level2_bubble_live: 이번 달(진행 중 포함) 즉시 판정 신호 (진입은 언제나 즉시).
        # is_level2_bubble: 실제 방어 행동 게이트. [원칙 4-조건A]에 따라 '2개월 연속' 100% 미만
        # 마감 전까지는 방어 모드를 유지하여 노이즈로 인한 휩소(왔다갔다)를 방지한다.
        ...
        # (1) 쿼터 벙커 스나이퍼 감시 (1순위: 전시 상황) — 원칙 3 V24.9: 4분할(75% 투입 : 25% 영구 보존)
        if qqq_mdd <= -0.15:
            msg += f"📉 **[쿼터 스나이퍼 기회] QQQ MDD {qqq_mdd_pct:.1f}%**\n"
            
            if qqq_mdd <= -0.45:
                msg += "💥 **시스템 위기 (2008년 리먼 사태 수준)**\n👉 **ACTION:** 스나이핑 시작 시점 총현금의 **25%** 투입 (누적 75% 화력 집행 완료)!\n🛡️ **방탄 벙커:** 남은 **25%는 양안전쟁/닷컴급 대비 영구 킵(절대 미투입)**하여 SGOV/BOXX에 보존!\n"
            elif qqq_mdd <= -0.35:
                msg += "🏦 **대세 하락장 (2020 코로나, 2022 바닥 수준)**\n👉 **ACTION:** 스나이핑 시작 시점 총현금의 **25%** 투입 (누적 50% 투입).\n"
            elif qqq_mdd <= -0.25:
                msg += "🌪️ **중급 하락장 (2018년 수준)**\n👉 **ACTION:** 스나이핑 시작 시점 총현금의 **15%** 투입 (누적 25% 투입).\n"
            else:
                msg += "📉 **일반적인 조정장**\n👉 **ACTION:** 스나이핑 시작 시점 총현금의 **10%** 투입.\n"
        ...
        # (2) RSI 및 이격도 광기 감시 (2, 3순위 - 스나이퍼가 아닐 때만 발동)
            ...
            if is_level2_bubble:
                _dev_note = "" if is_level2_bubble_live else f" (냉각 {qqq_dev_cool_months}/2개월)"
                trigger_str.append(f"QQQ 120월 이격도 {qqq_mo_dev*100:.1f}%{_dev_note}")
            
            trigger_msg = ", ".join(trigger_str)

            if is_level2_bubble:
                # [V24.6] 하이브리드 방어: 100% 현금이 아닌 '방어 비중(목표+20%p 현금)'으로 계속 매수 (전 Level 공통)
                msg += f"🚨 **[역사적 버블 경보] {trigger_msg} 돌파! (전 Level 공통)**\n"
                msg += "👉 **ACTION:** 자산 규모(Level) 불문, 목표 현금 비중에 **+20%p 추가 확보**하여 비상 현금 비중 설정.\n"
                msg += "👉 **ACTION:** 기존 보유 주식도 부족분만큼 즉시 비례 매도 (TQQQ/USD 현재 보유 비중대로, 50:50 강제 금지, 세금 최소화).\n"
                msg += "👉 **ACTION:** 신규 적립금 500만 원은 100% 현금이 아닌 '방어 비중(목표+20%p 현금)'에 맞춰 계속 기계적 매수 (FOMO 방지).\n"
                msg += "👉 **ACTION:** [해제] 이격도가 100% 미만으로 **2개월 연속** 마감되어야 평시 목표비중으로 정상화됩니다.\n"
            else:
                ...
                msg += "👉 **ACTION:** 신규 적립금 500만 원은 평시 목표비중대로 계속 매수 (이격도는 안정적이므로 방어 비중 전환 없음).\n"
        ...
        status_block = (
            f"📊 *Status Check*\n"
            f"• QQQ: ${qqq_price:.2f} │ 주봉RSI {qqq_rsi_wk:.1f} / 월봉RSI {qqq_rsi_mo:.1f} │ 120월 이격도 {qqq_mo_dev*100:.1f}% ({_qqq_ma120_str}, 냉각 {qqq_dev_cool_months}개월) │ MDD {qqq_mdd_pct:.2f}% (수정종가)\n"
